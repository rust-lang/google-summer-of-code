...
it is frequently hard to see what is going on when running miri on some code. Many miri errors are hard to debug, and using the built-in logging mechanisms produce an overwhelming amount of text.

thus we would like to restart this project from scratch. 

**Expected result**

a graphical frontend to miri that allows stepping through statements and inspecting some information not already trivially apparent from the command line miri. Integration into miri's CI to prevent bitrotting. Documentation about the design so any miri contributor can also keep priroda up to date. 
